...
interface Column { column: 'todos' | 'doing' | 'done' | null }

type DraggedItem = Task & Column

function App() {
  const [tasks, updateTasks] = useState<Tasks>({ todos: [], doing: [], done: []})
  ...
  const totalItems = () => tasks.todos.length + tasks.doing.length + tasks.done.length
  const addTodo = (event) => {
  ...
  const receiveItem = (event, newColumn : string) => {
		...
		if (newColumn == 'done') { enableConfetti(true) }
		dragItem({ task: '', column: null, id: 0 })
  }

  const reorderColumn = (event, target) => {
		const { column: targetColumn, id: targetId } = target 
		const { task, column, id } = dragged

		if (column != targetColumn) return;

		const currentDraggedIndex = tasks[column].findIndex(elem => elem.id === id);
		const targetIndex = tasks[column].findIndex(elem => elem.id === targetId);
		const updatedColumn = tasks[column].filter(({ id: taskId }) => taskId !== id)
